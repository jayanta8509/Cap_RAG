"""
FastAPI Document RAG Chatbot API
Provides REST endpoints for document Q&A using RAG with Pinecone
...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info("🚀 Starting Document RAG Chatbot API...")
    ...
    # Cleanup on shutdown
    logger.info("🛑 Shutting down Document RAG API...")

# Initialize FastAPI app
app = FastAPI(
    title="Document RAG Chatbot API",
    description="AI-powered document Q&A using Retrieval Augmented Generation with Pinecone",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
class ChatRequest(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "user_id": "user123",
                "query": "What information is available in the PDF documents?",
                "use_agent": False
            }
        }
    }
...
class ChatResponse(BaseModel):
    ...
    mode: str = Field(..., description="Processing mode used (counselor/agent/error)")
...
async def chat_endpoint(request: ChatRequest):
    ...
    **Parameters:**
    ...
    - **query**: User's message or question about the documents
    - **use_agent**: Set to true for complex queries requiring multi-step retrieval (default: false)
    
    **Features:**
    - User-specific conversation memory
    - Retrieval from CSV and PDF document sources
    ...
    if not system_initialized:
        raise HTTPException(
            status_code=503, 
            detail="Document RAG system is not initialized. Please check server logs."
        )
...
# Agent mode endpoint
@app.post("/chat/agent", response_model=ChatResponse, tags=["Chat"])
async def chat_agent_endpoint(request: ChatRequest):
    """
    Chat endpoint with agent mode enabled for complex queries
    
    This endpoint automatically uses agent mode for multi-step retrieval
    and comprehensive responses. Best for:
    - Complex document searches across multiple sources
